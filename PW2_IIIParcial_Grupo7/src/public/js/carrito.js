...
const mostrarCarrito = (data) =>{
    ...
    for(let i=0;i < data.length ;i++){  
        ...
        <td class="rows">
            <button onclick="handleSubtract(value)" value="${data[i].Productos.id_producto}" class="btnIcon" id="btnMinus">
                <span class="icon-minus"></span>
            </button>
                ${data[i].cantidad}
            <button class="btnIcon" id="btnPlus" onclick="handleAdd(value)" value="${data[i].Productos.id_producto}">
                <span class="icon-plus"></span>
            </button>
        </td>
        <td class="rows">
            <button onclick="handleEliminar(value)" value="${data[i].id_itemCarrito}" class="btnIcon" id="btnCancel">
...
const handleAdd= (data)=>{
    fetch(`http://localhost:6001/api/carrito/updateProduct/${data}/add`,{ method: 'PUT' });
    window.location.reload();
}
